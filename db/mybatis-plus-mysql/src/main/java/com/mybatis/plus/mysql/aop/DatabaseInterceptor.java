...
public class DatabaseInterceptor extends BaseMultiTableInnerInterceptor implements InnerInterceptor {
    ...
    public String handleSql(String sql) {
        ...
        if (statement instanceof Select) {
            ...
            processInsert(insert, 0, sql, null);
            return insert.toString();
        } else if (statement instanceof Update) {
            Update update = (Update) statement;
            processUpdate(update, 0, sql, null);
            return update.toString();
        } else if (statement instanceof Delete) {
            Delete delete = (Delete) statement;
            processDelete(delete, 0, sql, null);
            return delete.toString();
        } else {
            log.error("未识别的SQL类型: {}", sql);
            return sql;
        }
    ...
    protected void processInsert(Insert insert, int index, String sql, Object obj) {
        if (companyLineHandler.ignoreTable(insert.getTable().getName())) {
            // 过滤退出执行
            return;
        }
        ...
        String tenantIdColumn = companyLineHandler.getCompanyIdColumn();
        if (companyLineHandler.ignoreInsert(columns, tenantIdColumn)) {
            // 针对已给出租户列的insert 不处理
            return;
        }
        columns.add(new Column(tenantIdColumn));
        Expression tenantId = companyLineHandler.getCompanyId();
    ...
    protected void processUpdate(Update update, int index, String sql, Object obj) {
        ...
        if (companyLineHandler.ignoreTable(table.getName())) {
            // 过滤退出执行
            return;
        }
    ...
    protected void processDelete(Delete delete, int index, String sql, Object obj) {
        if (companyLineHandler.ignoreTable(delete.getTable().getName())) {
            // 过滤退出执行
            return;
        }
    ...
    protected void appendSelectItem(List<SelectItem<?>> selectItems) {
        ...
        selectItems.add(new SelectItem<>(new Column(companyLineHandler.getCompanyIdColumn())));
    }
    ...
    protected Column getAliasColumn(Table table) {
        ...
        column.append(companyLineHandler.getCompanyIdColumn());
    ...
    public void setProperties(Properties properties) {
        PropertyMapper.newInstance(properties).whenNotBlank("tenantLineHandler",
                ClassUtils::newInstance, this::setCompanyLineHandler);
    }
    ...
    public Expression buildTableExpression(final Table table, final Expression where, final String whereSegment) {
        ...
        return new EqualsTo(getAliasColumn(table), companyLineHandler.getCompanyId());
    }
}
